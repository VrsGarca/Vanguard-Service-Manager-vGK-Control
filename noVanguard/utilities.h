...
namespace utilities
{
	inline BOOL is_process_elevated()
	{
		...
		if (OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &h_token))
		{
			...
			CloseHandle(h_token);
			MessageBox(nullptr, L"GetTokenInformation failed. Program will close.", L"noVanguard", MB_ICONERROR | MB_OK);
			return FALSE;
		}

		MessageBox(nullptr, L"OpenProcessToken failed.Program will close.", L"noVanguard", MB_ICONERROR | MB_OK);
		return FALSE;
	}
